...
id_list = list(set(start_dict.keys())&set(stop_dict.keys()))
...
for item in tqdm(range(len(accept))):
    ...
    if strand == '+':
        ...
        if len(ds) > 0:
            exon_de = np.concatenate([exon[d[0]:d[1]+1] for d in ds])
        else:
            exon_de = np.zeros(1)-1
        for read in bamfile.fetch(chrom, exon[0], start_left): #5'UTR
            cs[0] += 1
        for read in bamfile.fetch(chrom, start_left, start_right): #Start
            cs[1] += 1
        for read in bamfile.fetch(chrom, start_right, stop_left): #CDS
            p = np.array(read.get_reference_positions())
            c = int(round(np.median(p)))
            if len(np.intersect1d(p, exon_de)) >0:
                cs[2] += 1
            else:
                cs[3] += 1
        for read in bamfile.fetch(chrom, stop_left, stop_right): #Stop
            cs[4] += 1
        for read in bamfile.fetch(chrom, stop_right, exon[-1]): #3'UTR
            cs[5] += 1
    else:
        for e in ex:
...
df.columns = ['5\'UTR','Start', 'DE', 'CDS', 'Stop','3\'UTR']
df.to_csv(outpath+'_regioncount_DE.csv')
